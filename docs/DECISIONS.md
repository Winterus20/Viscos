...
| 2026-06-20 | **PR-12 cross-ref (Auth C3 — token paste + MFA TOTP)** | `feat(auth): token paste + MFA TOTP auth flow (C3, no /auth/login)` (commit `1fbad6d`) — ADR-0011'in Varyant A encryption + MFA + token storage kararlarının production implementasyonu. Yeni `token::DiscordToken` newtype (`Secret<String>` wrapper + `ZeroizeOnDrop`-aware inner type) + `TokenStore` trait + `WindowsCredentialStore` (keyring-core 0.7 + DPAPI) ADR-0011 §Decision ile birebir uyumlu. `flow::AuthFlow` enum (`TokenPaste`, `MfaChallenge`) + `AuthManager` orchestrator, ADR-0011 §captcha handling (redirect + token paste) stratejisini somutlaştırır. `AuthError` +4 variant (`InvalidTokenFormat`, `NetworkError`, `StoreFailed`, `MfaFailed`, `#[non_exhaustive]`) — ADR-0007 thiserror/anyhow split'i korunur. 4 yeni IPC komutu (`AuthPasteToken`, `AuthValidateMfa`, `AuthLogout`, `AuthGetStatus`) — ADR-0012 §IPC contract ile uyumlu (Rust tarafı kaynaktır). **`POST /auth/login` kasıtlı olarak atlanır**: Cloudflare+hCaptcha korumalı + undocumented + self-bot ToS violation riski; twilight-rs zaten dahil etmiyor (ADR-0008 §Alternatifler). 81 test pass, yeni external dependency yok (`totp-rs`, `zeroize`, `secrecy`, `keyring-core` zaten workspace'de). |
...
| 2026-06-20 | **PR-9 cross-ref (frontend IPC types derived from Rust contract)** | `feat(ipc): TypeScript bridge types derived from Rust IPC contract` (commit `4b0430a`) — ADR-0012 §2 (Bridge.ts Kırılganlık Azaltma + IPC contract tek taraflı yazım yasağı) production implementasyonu. `frontend/src/ipc/{types,commands,events}.ts` dosyaları Rust kanonik enum'larından (`crates/viscos-ipc/src/{types,command,event}.rs`, PR-5) derive edildi: 13 `IpcCommand` variant, 8 `IpcEvent` variant, `WatchdogKind` enum. `frontend/src/bridge.ts` inline type tanımlarından arındırıldı — `re-exports from ./ipc/* submodules`. Serde wire format korundu (`{"type": "<Variant>", "data": {...}}` / `{"kind": "<Variant>", "payload": {...}}`) — ADR-0012 §IPC contract uyumlu. `crates/viscos-ipc/src/lib.rs` codegen hint comment'i eklendi. ADR-0008 (twilight) ile çakışma yok (twilight kendi IPC'sini transport'ta, viscos IPC'sini shell↔webview arasında). Touch radius: 5 dosya, +293/-26. Refs: ADR-0012 §2, ADR-0008 §IPC. |
| 2026-06-20 | **PR-11 cross-ref (Win11 CEF default — Faz 1.6 Dalga 1c)** | `feat(webview): Win11 CEF default rollout Dalga 1c (telemetry-driven)` (commit `83c4866`) — ADR-0012 §4 B1 (Win11'de CEF default) + Faz 1.6 Dalga 1c production implementasyonu. `resolve_backend(cli, config, telemetry: Option<&TelemetryStore>)` üç parametreli signature: RDP → WebView2; Win10/altı → WebView2; Win11 + TelemetryStore::Required (GDI peak ≥ 8500) → CEF; Win11 + Optional → WebView2; Win11 + Unknown/None → CEF default (ADR-0012 §4 B1). 6 yeni `resolve_backend_priority` test senaryosu (CLI > config > auto). `cef-backend` feature kapalıyken `BackendKind::Cef` seçilse bile runtime `ViscosError::Unimplemented` döner — PR-2 stub fallback ile semptomatik symmetry. Telemetry wiring `main.rs`'de `None` Faz 1.5 Polish'e kadar (placeholder). `Cargo.lock` +1 (viscos-telemetry ekle), `crates/viscos-webview/Cargo.toml` +1 (viscos-telemetry dep). Touch radius: 11 dosya, +248/-82. Refs: ADR-0012 §4, ADR-0008 (telemetry contract unchanged). |
...
|| 2026-06-19 | **PR-7 merge: hotfix — distribution profile test (feature-blind)** | `fix(distribution): heap_profiling_enabled test was feature-blind` (commit `c10deb2`). CI'nin `--all-features` modunda `viscos-distribution`'un `dhat` feature'ı aktifken test koşullu `assert!(!heap_profiling_enabled())` ile yanlışlıkla fail veriyordu. `cfg!(feature = "dhat")` ile semptomatik symmetry sağlandı. PR-1..PR-6'nın yeşile dönmesini sağlayan MVP-1B blocker kaldırıldı. Touch radius: 1 dosya, 5+/2-. Refs: CI runs 27837049406 / 27837067625 / 27837105255 / 27837144962. |
|| 2026-06-20 | **PR-8 merge: PR-4 squash follow-up (Cargo.lock refresh)** | `chore: refresh Cargo.lock after PR-4 squash merge` (commit `0288715`). PR-4'ün squash merge'i sonrası workspace'in `Cargo.lock` dosyası `viscos-shell` ve bağımlılıklarının (iced 0.14, global-hotkey, tao, tray-icon, parking_lot, wasapi) yeni sürümleriyle yenilendi. 241 satır Cargo.lock diff; kod değişikliği yok. Refs: cursorrules §10 commit mesajı standardı (her commit'te `Co-authored-by:` insan adı zorunlu). |
|| 2026-06-20 | **PR-9 merge: frontend IPC types derived from Rust contract** | `feat(ipc): TypeScript bridge types derived from Rust IPC contract` (commit `4b0430a`). `frontend/src/ipc/{types,commands,events}.ts` dosyaları Rust kanonik enum'larından (`crates/viscos-ipc/src/{types,command,event}.rs`, PR-5) derive edildi: 13 `IpcCommand` variant, 8 `IpcEvent` variant, `WatchdogKind` enum. `frontend/src/bridge.ts` inline type tanımlarından arındırıldı. Serde wire format korundu (`{type, data}` / `{kind, payload}`). ADR-0012 §2 (selector resilience + bridge.ts kuralları) ile uyumlu — tek taraflı yazım yasak, Rust kaynaktır. Refs: ADR-0012 §2, ADR-0008 (twilight IPC entegrasyonu). |
|| 2026-06-20 | **PR-10 merge: shell refactor — hotkeys.rs/window.rs split into submodules** | `refactor(shell): split hotkeys.rs and window.rs into submodules` (commit `14f10ae`). cursorrules §2 dosya boyutu sınırı (>400 satır refactor, >600 satır zorunlu refactor) gereği: `hotkeys.rs` (536 satır) → `integration/hotkeys/{types,manager,controller,mod}.rs` (179+170+119+107 satır); `window.rs` (502 satır) → `window/{config,tray,shell,mod}.rs` (192+141+135+68 satır). Davranış değişikliği yok, `pub use` re-export hub ile public API korundu. Touch radius: 10 dosya, tüm yeni dosyalar <200 satır. Refs: cursorrules §2. |
|| 2026-06-20 | **PR-11 merge: Win11 CEF default rollout (Faz 1.6 Dalga 1c)** | `feat(webview): Win11 CEF default rollout Dalga 1c (telemetry-driven)` (commit `83c4866`). `resolve_backend(cli, config, telemetry: Option<&TelemetryStore>)` signature: RDP → WebView2; Win10/altı → WebView2; Win11 + TelemetryStore::Required (GDI peak ≥ 8500) → CEF; Win11 + Optional → WebView2; Win11 + Unknown/None → CEF default (ADR-0012 §4 B1). 6 yeni `resolve_backend_priority` test senaryosu. `cef-backend` feature kapalıyken `BackendKind::Cef` seçilse bile runtime `ViscosError::Unimplemented` döner (PR-2 stub fallback ile uyumlu). Telemetry wiring `main.rs`'de `None` Faz 1.5 Polish'e kadar. Touch radius: 11 dosya, +248/-82. Refs: ADR-0012 §4, ADR-0008 (telemetry contract unchanged). |
|| 2026-06-20 | **PR-12 merge: Auth token paste + MFA TOTP (C3 — `/auth/login` deliberately bypassed)** | `feat(auth): token paste + MFA TOTP auth flow (C3, no /auth/login)` (commit `1fbad6d`). Discord `POST /auth/login` Cloudflare+hCaptcha korumalı + undocumented + self-bot ToS violation riski taşıdığı için **kasıtlı olarak atlanır** (twilight-rs zaten dahil etmiyor, ADR-0008/ADR-0011). Yeni `token::DiscordToken` newtype (`Secret<String>` wrapper, `ZeroizeOnDrop`-aware) + `TokenStore` trait + `WindowsCredentialStore` (keyring-core 0.7 + DPAPI). `flow::AuthFlow` enum (`TokenPaste`, `MfaChallenge`) + `AuthManager` orchestrator. `AuthError` +4 variant (`InvalidTokenFormat`, `NetworkError`, `StoreFailed`, `MfaFailed`, `#[non_exhaustive]`). 4 yeni IPC komutu: `AuthPasteToken`, `AuthValidateMfa`, `AuthLogout`, `AuthGetStatus`. Yeni external dependency yok (`totp-rs`, `zeroize`, `secrecy`, `keyring-core` zaten workspace'de ADR-0011). Touch radius: 5 dosya, +699/-2, 81 test pass. **Human Decision Required: YES** (new public API + `/auth/login` bypass kararı). Refs: ADR-0011, ADR-0008, ADR-0012. |

---

## PR → ADR Cross-Reference Tablosu (PR-1..PR-12)

| PR | Branch | Commit | Date | Description | ADR Refs | Merge Commit |
|----|--------|--------|------|-------------|----------|--------------|
| PR-1 | `feat/telemetry-store-mvp3` | `175b49c` | 2026-06-19 | Telemetry store (SQLite GDI + restart events) | ADR-0010 §Adaptive, ADR-0012 §4 B1 | `175b49c` |
| PR-2 | `feat/webview-webview2-runtime` | `9d2f4d1` | 2026-06-19 | WebView2 runtime + CEF feature-gated stub | ADR-0012 §1, §6 | `9d2f4d1` |
| PR-3 | `feat/cache-facade-repository` | `cc8933e` | 2026-06-19 | Cache facade + repository pattern | ADR-0010 §Decision, §Decision Eki | `cc8933e` |
| PR-4 | `feat/shell-hotkey-audio-scaffold` | `fa3d865` | 2026-06-19 | Shell hotkey/audio/tray/panel scaffold | ADR-0012 §1, §2 (bridge resilience) | (squash → `0288715`) |
| PR-5 | `feat/api-gateway-cache-bridge` | `9065880` | 2026-06-19 | Gateway + cache bridge + IPC types | ADR-0008 §Twilight, ADR-0010 §Repository | `9065880` |
| PR-6 | `chore/meta-docs-ci-installer-audit` | `81cb003` | 2026-06-19 | Meta + CI workflows + WiX + WinGet + audit | ADR-0004 §CI, ADR-0012 §Release | `81cb003` |
| PR-7 | `fix/distribution-profile-test` | `c10deb2` | 2026-06-19 | Hotfix: `heap_profiling_enabled` feature-blind test | ADR-0004 §CI matrix | `c10deb2` |
| PR-8 | (PR-4 squash follow-up) | `0288715` | 2026-06-20 | Cargo.lock refresh after PR-4 squash | (no ADR change — workspace hygiene) | `0288715` |
| PR-9 | `feat/frontend-ipc-bridge-types` | `4b0430a` | 2026-06-20 | TypeScript bridge types derived from Rust IPC | ADR-0012 §2 (bridge.ts), ADR-0008 §IPC | `4b0430a` |
| PR-10 | `refactor/shell-split-hotkeys-window` | `14f10ae` | 2026-06-20 | Split `hotkeys.rs`/`window.rs` into submodules | (cursorrules §2 — no ADR change) | `14f10ae` |
| PR-11 | `feat/webview-cef-default-win11-dalga-1c` | `83c4866` | 2026-06-20 | Win11 CEF default rollout (telemetry-driven) | ADR-0012 §4 B1, ADR-0008 §Telemetry contract | `83c4866` |
| PR-12 | `feat/auth-token-paste-mfa-totp` | `1fbad6d` | 2026-06-20 | Auth token paste + MFA TOTP (C3, no `/auth/login`) | ADR-0011, ADR-0008, ADR-0012 | `1fbad6d` |

**Not:** Commit hash'ler `git log` ile doğrulanmıştır (2026-06-20 itibarıyla `main` branch, HEAD = `e4e82ae`). PR-4 squash merge'i nedeniyle PR-4 ile PR-8 aynı içeriğe işaret eder; PR-8 sadece `Cargo.lock` refresh'idir.
